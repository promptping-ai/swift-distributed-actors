...
extension ActorID: Comparable {
    public static func < (lhs: ActorID, rhs: ActorID) -> Bool {
        lhs.node < rhs.node || (lhs.node == rhs.node && lhs.path < rhs.path) || (lhs.node == rhs.node && lhs.path == rhs.path && lhs.incarnation < rhs.incarnation)
    }
}
...
public struct ActorPathSegment: Hashable, Sendable {
    ...
    static func validatePathSegment(_ name: String) throws {
        ...
        func isValidASCII(_ scalar: Unicode.Scalar) -> Bool {
            (scalar >= ValidActorPathSymbols.a && scalar <= ValidActorPathSymbols.z) || (scalar >= ValidActorPathSymbols.A && scalar <= ValidActorPathSymbols.Z)
                || (scalar >= ValidActorPathSymbols.zero && scalar <= ValidActorPathSymbols.nine) || ValidActorPathSymbols.extraSymbols.contains(scalar)
        }
